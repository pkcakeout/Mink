import os,sys,imp,subprocess,time,shutil, StringIO, ConfigParser
import boto
...
'''
def readConfigFileFromS3(bucket,algoDir):
    ...
    try:
        config['cluster_size'] = conf.get(sectionName, 'cluster_size')
    except:
        print('No cluster size given... Taking: 2')
        config['cluster_size'] = 2

    #Read the AMI ID
    try:
        config['worker_image_id'] = conf.get(sectionName, 'worker_image_id')
    except:
    ...
    try:
        config['worker_instance_type'] = conf.get(sectionName, 'worker_instance_type') #instance type
    except:
        print('No worker instance type given... Taking: m1.small')
        config['worker_instance_type'] = 'm1.small'

    try:
        config['algorithm_version'] = conf.get(sectionName, 'algorithm_version') #instance type
    except:
        print "algorithm_version not specified, unable to determine version %s" %algoDir

    try:
        config['min_cores'] = conf.get(sectionName, 'min_cores') #instance type
    except:
        print('No minimum cores given... Taking: 1')
        config['min_cores'] = 1

    try:
        config['min_RAM'] = conf.get(sectionName, 'min_RAM') #instance type
    except:
        print('No minimum RAM given... Taking: 8')
        config['min_cores'] = 8

    return config

def readConfigFileLocal(algoDir):
    configFile = os.path.join(algoDir+"/config")
    if not os.path.isfile(configFile):
        print "config file not found"
        return False
    configObj = open(configFile)
    contents = configObj.read()
    configObj.close()
    buf = StringIO.StringIO(contents)
    conf = ConfigParser.ConfigParser()
    conf.readfp(buf)
