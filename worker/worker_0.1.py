...
class Algorithms:
    ...
    def compareAlgorithmVersion(self, algorithmName):
        configS3 = readConfigFileFromS3(connectToS3Bucket(self.s3bucket, self.region), 'algorithms/'+algorithmName)
        configWorker = readConfigFileLocal(os.path.join(APP_FOLDER,"algorithms",algorithmName))
        if configS3 and configWorker:
            if 'algorithm_version' in configS3.keys() and 'algorithm_version' in configWorker.keys():
                return True if configS3['algorithm_version'] == configWorker['algorithm_version'] else False
        return False
    ...
    def downloadAlgorithms(self):
        ...
        for algorithm in availableAlgorithms:
            ...
            if (".7z" in algorithmZip) or (".zip" in algorithmZip):
                ...
                configKey = bucket.get_key(os.path.dirname(algorithmZip)+"/config")
                if configKey and os.path.isfile(os.path.join(self.app_folder,'algorithms',algorithmName,'config')):
                    algorithmUpToDate = self.compareAlgorithmVersion(algorithmName)
                else:
                    print("Unable to compare algorithm version %s..." % algorithmName)
                    algorithmUpToDate = False
                if fileKey and not(os.path.exists(os.path.join(self.app_folder,'algorithms',algorithmName))) or not algorithmUpToDate:
